...
// ResultsView · "Did it work?" The outcome-first view. A judge (or depositor)
...
// wallet needed; this is the view the demo ends on. Extracted so it can render
// standalone (full page) or embedded as a tab.
...
export function ResultsView({
  ...
  const [policyId, setPolicyId] = useState<string | null>(
    propPolicyId !== undefined ? propPolicyId : null,
  );
  const [now, setNow] = useState(() => Date.now());

  // Policy resolution · if a policyId is passed in (even null), use it
  // directly; otherwise fall back to the URL + latest trader identity. Re-runs
  // whenever the incoming prop changes.
  useEffect(() => {
  ...
  return embedded ? content : <main className="min-h-screen bg-bg text-ink">{content}</main>;
}
