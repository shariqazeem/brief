...
// BrainView · the Operator Brain surface, extracted into a reusable component.
// Renders one decision at a time, read like a pilot's black box: What I Saw →
// What I Remembered → What Could Go Wrong → Execution Quality → Policy Check →
// Decision → Outcome.
//
// Standalone at /brain (read-only, public, no wallet) and embeddable as a tab.
// When `embedded` is true it drops the full-page chrome (no <main> min-h-screen
// wrapper, no eyebrow/back-nav header) so it can live inside another surface.
...
export function BrainView({
  ...
  useEffect(() => {
    // If a policyId prop is supplied (embedded/controlled), use it directly.
    // Otherwise fall back to the ?policy= URL param and latest local identity.
  ...
  const content = (
    <div className="mx-auto max-w-2xl px-5 py-10 sm:px-8 sm:py-14">
      {/* Eyebrow + judge-path nav · page chrome only, dropped when embedded */}
      {!embedded && (
