...
// EvolutionView · the fourth pillar: the operator getting *better* over time.
...
//
// Extracted from src/app/evolution/page.tsx so it can be embedded as a tab.
// When `embedded` is true, the outer full-page chrome (the min-h-screen <main>
// wrapper + the eyebrow/back-nav header row) is omitted; otherwise it renders
// EXACTLY as the standalone /evolution page does.
...
export function EvolutionView({
  ...
  embedded,
}: { policyId?: string | null; embedded?: boolean } = {}) {
  const [policyId, setPolicyId] = useState<string | null>(null);
  useEffect(() => {
    // When a policyId is supplied by the parent (embedded as a tab), use it
    // directly and re-resolve whenever it changes. Otherwise fall back to the
    // ?policy= URL param, then the latest local trader identity.
    if (propPolicyId !== undefined) {
      setPolicyId(propPolicyId);
      return;
    }
  ...
  const content = (
    <div className="mx-auto max-w-2xl px-5 py-10 sm:px-8 sm:py-16">
      {/* eyebrow + nav · omitted in embedded mode */}
      {!embedded && (
  ...
  // Embedded (as a tab): return the bare content container, no page chrome.
  if (embedded) return content;

  // Standalone /evolution page: full-page chrome, EXACTLY as before.
  return <main className="min-h-screen bg-bg text-ink">{content}</main>;
}
